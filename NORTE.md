...
**Anclas vigentes (actualizado 2026-08-12, post D-150/D-151):** Cuadre 0.00 · seg 0/0/0 · cargas 85 · flags 1 · aplicaciones 212 · CxC 565,985.13 · CxP 504,869.78 · JPM 43,308.54 · folio_max JPM 401.

**Próximo horizonte (actualizado 2026-08-12):** 🔵 Rediseño de arquitectura: Operación (OP) como hilo conductor — ver `ARQUITECTURA-OPERACION.md`. Modelo aprobado. **Backend COMPLETO — Fase 0/1a/2/alta-módulo/3a/3b/3c/3d cerradas** (D-140..D-151: tabla `operaciones` + backfill 85 OPs + vistas de lectura + pantalla "Operaciones (OP)" en producción + las 4 RPCs de captura `fn_abrir_operacion`/`fn_op_agregar_venta`/`fn_op_agregar_compra`/`fn_op_agregar_embarque`). **Lo único que falta es FRONTEND de captura:** Slice 1 "+ Nueva operación" ya cableado (E113); **faltan Slice 2 "Agregar compra"** y **Slice 3 "Registrar embarque"** (toca costos — la de más cuidado) — ver `PENDIENTES-BACKEND.md`.
