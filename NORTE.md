...
**Rediseño OP — CERRADO (actualizado 2026-08-12):** 🟢 Operación (OP) como hilo conductor — ver `ARQUITECTURA-OPERACION.md`. **Backend y frontend COMPLETOS, probado end-to-end EN PRODUCCIÓN** (D-140..D-154; Slices 1→2→3 de captura, E113-E116): tabla `operaciones` + backfill 85 OPs + vistas de lectura (con LEFT JOIN a cargas, D-153, para que una OP sin embarque todavía se vea) + pantalla "Operaciones (OP)" + flujo completo "+ Nueva operación" → "Agregar compra" → "Registrar embarque" (4 RPCs: `fn_abrir_operacion`/`fn_op_agregar_venta`/`fn_op_agregar_compra`/`fn_op_agregar_embarque`, con el fix D-154 de ambigüedad de columna ya aplicado). La prueba se hizo con **OP-0086** (creó OP + venta + compra OC-0004 + carga P-095, movió CxP +25,000 / CxC +27,000) y se **ANULÓ LIMPIA** con `fn_anular_carga`/`fn_anular_orden`/`fn_cancelar_so` — anclas de vuelta a cargas 85 · CxP 504,869.78 · CxC 565,985.13 · Cuadre 0.00. Queda un **backlog de ajustes (A-E)** (herencia compra→embarque, editar operaciones, UX de herencia en el embarque, atribución de costo, auto-liga carga↔venta) sin dueño asignado — ver `PENDIENTES-BACKEND.md`.
