...
_Última actualización: post-autosuficiencia (2026-08-09) — plan Fase 1→3 CERRADO Y DESPLEGADO + D-138 (Tema 1) + D-139 (fn_alta_producto). Ver sección "Actualización E102-E106 + post-autosuficiencia" al final. **Verificar SIEMPRE en vivo** antes de tocar nada: el otro chat también puede haber escrito._
...
- **Backlog** (consignación cobrada sin liquidar), al cierre E86: 4 productores / 11 cargas / $54,224.70. **Actualizado 2026-08-09: 4 productores / 10 cargas / $44,224.70** — Cornejos **P-043 ($10,000) ya se liquidó**; queda Cornejos P-047 ($11,571.20), Carrifoods 6 ($20,329.50), Akambarhu P-073/075 ($11,874), Agrofepac P-071 ($450). Todas >15 días (rojo).
...
_Frontend E90-E101 (rediseño "Operador estilo Silo") sigue documentado aparte en `SISTEMA-DISEÑO.md`/`REPORTE-FRONTEND.md` — esta sección cubre solo el trabajo de BACKEND que faltaba sincronizar aquí: la sesión de "autosuficiencia" (E102-E106, D-120..D-137) y su cierre (D-138/D-139). Detalle completo y contratos vivos en `PENDIENTES-BACKEND.md` y `MAPA-CAPTURA.md` — este resumen es para que NORTE.md no se quede desincronizado._

**Objetivo de la sesión:** que lo rutinario (captura + catálogos) se opere 100% desde el sistema, sin depender de que el chat de backend lo haga a mano — dejando el motor contable protegido.

- **Tema 2 — Línea de proyecto + socios (D-120..D-124):** `fn_ajustar_linea_proyecto` (money-neutral, candado ≥ dispuesto), `fn_registrar_aportacion_socio` (genérica, 3 naturalezas: préstamo sin interés / financiamiento con tasa / custodia), origen de fondeo opcional en `fn_anticipo_productor` (retrocompatible), balance "Deuda a socios" generalizado + `v_deuda_socios`. Frontend: Ajustar línea (ficha Proyecto), + Aportación de socio (Tesorería/Proyecto), selector Origen (Anticipo). **Miguel ya opera esto solo** (folio 397, PRJ-005).
- **P1 — Directorio autosuficiente (D-125..D-128):** `contrapartes.recibe_pagos` + `capturado_por`/`capturado_ts`; picker de "Registrar gasto" desacoplado de `clase` (Samuel/Juan ya salen en Sueldo); `fn_alta_contraparte`/`fn_editar_contraparte` con `recibe_pagos` + rastro; vista `v_directorio_contrapartes`.
- **P2 — Catálogos autoservibles (D-129..D-132):** Conceptos de costo (`fn_alta_concepto_costo`/`fn_editar_concepto_costo`); Cuentas (`tipo` banco/virtual, `fn_alta_cuenta`/`fn_editar_cuenta` solo permite banco real).
- **Fase 1 — Puerta única de captura (D-133):** `fn_traspaso` saneado (gate + `fn_actor` + folios por rango). Frontend: botón "+ Registrar" con chooser de intención (Cobro/pago · Gasto · Anticipo · Aportación · Traspaso) reemplaza los 4 botones sueltos de Tesorería.
- **Fase 2a — Categoría de deducción (D-134):** tabla `categorias_deduccion` + `fn_alta_categoria_deduccion`/`fn_editar_categoria_deduccion` + vistas `v_categorias_deduccion`/`_admin`; `liquidacion_deducciones.categoria` pasó de CHECK fijo a FK al catálogo.
- **Fase 2b — Categorías de gasto (D-135):** `tipos_movimiento.activo`; `fn_alta_categoria_gasto` (clona el comportamiento del grupo `gasto_operativo`/`gasto_financiero`, candado contra tipos estructurales) + `fn_editar_categoria_gasto` (activar/desactivar) + vistas `v_categorias_gasto`/`_admin`.
- **Fase 2c — Editar productos/variedades (D-136, D-137):** `fn_editar_producto`/`fn_editar_variedad` + permisos de las 5 RPCs de producto/variedad unificados a `capturar` (2 no tenían gate — hueco cerrado). **✅ Confirmado en producción por Miguel** (capturas de pantalla): botón "Editar" en Catálogos → Productos (nombre, código de ítem, Activo).
- **Fase 3 — limpieza:** moneda (USD/MXN) centralizada en `ERP.MONEDAS` (antes duplicada en 3 archivos frontend). Seeds confirmados como enums de sistema (documento/evento/presupuesto) — no requieren panel de usuario. Estado de Load queda como enum (no es catálogo de usuario).

**Cierre post-plan:**
- **D-138 — Tema 1, filtro de "Aplicar a carga" (backend + frontend, cerrado y desplegado):** vista nueva `v_carga_contrapartes` (folio_carga, contraparte_id, contraparte_nombre, rol['cliente'|'proveedor'|'costo']) — une cliente/proveedor de encabezado + contrapartes de línea de costo. Cubre proveedores de SERVICIO (flete/comisión/reempaque, ej. SUAREZ BROKERAGE en P-076) que antes se perdían porque el filtro comparaba por nombre solo contra el encabezado (`v_carga_detalle`). `modulo-tesoreria.js` → `editarMovimiento()` ahora filtra por ID contra la vista nueva. Verificado en vivo (4 casos), `node --check` limpio.
- **D-139 — `fn_alta_producto` consolidada (backend):** una sola firma `(p_nombre, p_codigo_item)` con detección de duplicado exacto (RAISE) y "parecido" (warning en `data[0].advertencia`). Antes había 2 firmas vivas (1-arg con la validación pero sin call-site; 2-arg sin validación, la que sí usaba el front) — el aviso de "parecido" nunca se mostraba en producción por ese desalineamiento. Corregido sin tocar frontend.
- **`documentos.entidad='load'`:** confirmado que el CHECK constraint de `documentos` ya incluye `'load'` desde D-71 (E76) — soporte preexistente, solo faltaba documentarlo (la incertidumbre vivía en un comentario de `modulo-loads.js`).
...
**Anclas vigentes (actualizado 2026-08-12, post D-154):** Cuadre 0.00 · seg 0/0/0 · cargas 85 · CxC 565,985.13 · CxP 504,869.78 · JPM 43,308.54 · seq_operaciones_num 86.

**Rediseño OP — CERRADO (actualizado 2026-08-12):** 🟢 Operación (OP) como hilo conductor — ver `ARQUITECTURA-OPERACION.md`. **Backend y frontend COMPLETOS, probado end-to-end EN PRODUCCIÓN** (D-140..D-156; Slices 1→2→3 de captura, E113-E116): tabla `operaciones` + backfill 85 OPs + vistas de lectura (con LEFT JOIN a cargas, D-153, para que una OP sin embarque todavía se vea, y con D-155 filtrando las cargas/OPs anuladas para que no ensucien la lista ni el header) + pantalla "Operaciones (OP)" + flujo completo "+ Nueva operación" → "Agregar compra" → "Registrar embarque" (4 RPCs: `fn_abrir_operacion`/`fn_op_agregar_venta`/`fn_op_agregar_compra`/`fn_op_agregar_embarque`, con el fix D-154 de ambigüedad de columna ya aplicado). Además, ya está vivo `fn_anular_operacion` (D-156): anula todo el hilo de una OP (venta+compra+embarque) de un jalón, requiere permiso administrar + motivo obligatorio, cableado en el detalle de la OP. La prueba se hizo con **OP-0086** (creó OP + venta + compra OC-0004 + carga P-095, movió CxP +25,000 / CxC +27,000) y se **ANULÓ LIMPIA** con `fn_anular_carga`/`fn_anular_orden`/`fn_cancelar_so` — anclas de vuelta a cargas 85 · CxP 504,869.78 · CxC 565,985.13 · Cuadre 0.00. Queda un **backlog de ajustes (A-E)** (herencia compra→embarque, editar operaciones, UX de herencia en el embarque, atribución de costo, auto-liga carga↔venta) sin dueño asignado — ver `PENDIENTES-BACKEND.md`.
