...
_Documento de diseño. Congela el modelo acordado ANTES de tocar código. Última actualización: 2026-08-10 (borrador inicial, sesión de rediseño con Miguel). Estado: **PROPUESTA — pendiente de aprobación final de Miguel antes de Fase 0**._
...
## 8. Preguntas abiertas (a resolver antes de Fase 0)

- **Numeración `OP-XXXX`:** ¿arranca en OP-0001 y se hace backfill de las 85 en orden de fecha de carga? ¿O se respeta el folio P-XXX viejo como referencia histórica dentro de la OP? (Propuesta: OP nueva secuencial + guardar `folio_carga_v1` como etiqueta histórica.)
- **Consignación y el momento del CxC:** confirmar que el reconocimiento al cobro (no al embarque) se modela como estado de la venta, no como excepción.
- **Comisión pura:** confirmar que la OP puede existir SIN capa de compra (Plein no compra) sin romper ninguna vista de CxP.
- **¿La OP reemplaza al "Proyecto"** (proyectos_productor / anticipos) o conviven? Hoy los anticipos a productor cuelgan de proyecto, no de carga. Definir la relación OP↔Proyecto.

---

## 9. Estado y siguiente paso

**Estado:** PROPUESTA. Modelo conceptual aprobado por Miguel (sesión 2026-08-10, con ejemplo real P-011). Falta: aprobación del documento completo + resolver §8 antes de arrancar Fase 0.

**Siguiente paso al aprobar:** resolver las 4 preguntas de §8, luego GATE de Fase 0 (andamiaje vacío, reversible).
