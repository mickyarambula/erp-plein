...
- **Responsive es requisito permanente (E131/D-193): UNA sola base de código, todo módulo NUEVO nace responsive.** No crear app/diseño móvil aparte. Breakpoints fijos (convención, ver `SISTEMA-DISENO.md` §13): móvil `@media(max-width:640px)`, tablet `@media(max-width:1024px)`, desktop `>1024px` — no inventar valores sueltos. Tablas de 8+ columnas → usar el patrón tabla→tarjeta: llamar `ERP.marcarTabla(cont)` tras pintar cada tabla (copia `<th>`→`data-label`, el CSS `.tabla-cards` apila las filas en móvil). Formularios: 1 columna en móvil, inputs a 16px (evita zoom iOS) + área táctil ~44px. Verificar en iPhone (~390px) además de desktop: cero scroll horizontal, todo tocable con el pulgar.
