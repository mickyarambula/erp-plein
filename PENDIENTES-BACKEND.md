...
- ✅ **RESUELTO (D-190, cerrado 2026-08-18 por backend) — módulo `'o3-compras'` dado de alta en
  `modulos_erp` + `rol_modulos`:** confirmado en la sesión D-192 — el menú ya trae "Compras" junto
  con el resto de Camino C (Customer PO, Sales Orders, Inventario) sin depender de URL directa. El
  mismo backend también limpió el menú: 24 módulos legacy quedaron ocultos, solo quedan visibles
  Inicio, Camino C (5 módulos) y Usuarios.
