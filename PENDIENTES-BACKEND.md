...
- ⬜ **Fase 3c (Slice 2 frontend, "Agregar compra"):** `fn_op_agregar_compra` (envuelve `fn_crear_orden_compra`: folio interno OC-XXXX + número oficial; se salta en comisión pura). Money-neutral.
- ⬜ **Fase 3d — LA DE MÁS CUIDADO (Slice 3 frontend, "Registrar embarque"):** `fn_op_agregar_embarque` (envuelve `fn_crear_carga`: embarque + lote + costos, con herencia de venta/compra). **TOCA COSTOS → GATE+ENSAYO fila por fila.**
